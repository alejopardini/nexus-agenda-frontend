...
export default function RegistroProfesional() {
  ...
  const { registerProfesional } = useAuth()
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await registerProfesional(payload)
      navigate('/')
    } catch (err) {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-slate-100">
      ...
      >
        <h1 className="text-xl font-bold mb-1 text-slate-800">Registro de profesional</h1>
        <p className="text-sm text-slate-500 mb-6">
          Tu cuenta es tuya, no de una clínica en particular. Después podés sumarte a una (o varias) aceptando invitaciones.
        </p>
        ...
        <input
          ...
          className="w-full border border-slate-300 rounded px-3 py-2 mb-1" required
        />
        <p className="text-xs text-slate-400 mb-4">
          Las clínicas te van a invitar buscándote por este email.
        </p>
        ...
        >
          {guardando ? 'Creando...' : 'Crear cuenta de profesional'}
        </button>
